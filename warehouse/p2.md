...
Keywords: heart disease, decision tree, random forest, support vector machine,machine learning, prediction.
...
![workflow.ed.p.01.png](https://github.com/otainginga/Image/blob/main/workflow.ed.p.01.png?raw=true)
...
### ED.P.Cross Validation
 ...
 To avoid data bias from a single partitioned dataset and to fully use the data, this study uses 10-fold cross-validation to eliminate this aspect. By dividing the whole data into 10 copies and taking one copy at a time without duplication as the test set, while using the other 9 copies as the training set to train the model, we will take the amount of 10 biases obtained and average them to get the result as the evaluation result, which will be the closest to the real performance of the model, and it will be used to screen the model.
